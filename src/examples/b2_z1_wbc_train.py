"""Train a B2 + Z1 whole-body end-effector reaching policy.

Same adaptive reward scheduling as ``b2_train.py``:

Phase 1 — Stability
    ``target_coeff = 0``. Only stability rewards are active (base height,
    orientation, action rate, arm posture, self-collision keep-out). The robot
    first learns to stand upright with the arm riding along.

Phase 2 — Reaching
    Once the per-iteration stability reward plateaus, ``target_coeff`` ramps
    ``0 -> max_target_coeff`` so ``tracking_ee`` starts dominating. The robot
    then learns to walk to the world-frame ball and touch it with the Z1
    end-effector.

Usage::

    uv run src/examples/b2_z1_wbc_train.py -e b2-z1-wbc \\
        --backend gpu --device cuda -B 4096 --max_iterations 1500
"""
...
from b2_z1_wbc_env import B2Z1WholeBodyEnv, default_cfgs


# =========================================================================
# Plateau detector (same as dynamic_ics_dog_train.py / b2_train.py)
# =========================================================================
    ...
    for it in range(start_iter, tot_iter):
            ...
            for ep_info in ep_infos:
                for key, val in ep_info.items():
                    if not key.startswith("rew_"):
                        continue
                    # Exclude the ramped tracking term from the stability signal.
                    if "tracking_ee" in key:
                        continue
        ...
        if it >= min_iterations and it % check_interval == 0:
            if phase == "stability":
                ...
                if plateau_count >= plateau_patience:
                    ...
                    print(
                        f"\n{'=' * 60}\n"
                        f" PHASE TRANSITION: stability -> reaching  (iter {it})\n"
                        f"{'=' * 60}\n"
                    )
            if phase == "reaching" and env.target_coeff < max_target_coeff:
                env.target_coeff = min(env.target_coeff + coeff_increment, max_target_coeff)
                print(f"  [reaching] target_coeff = {env.target_coeff:.3f}")
        ...
        if it == start_iter:
            try:
                ...
                print(f"[b2_z1_wbc_train] skipping git snapshot: {exc}")
                git_file_paths = []
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="gpu",
        choices=["cpu", "gpu"],
        help="Genesis backend. Use 'cpu' for smoke-tests without CUDA.",
    )
    parser.add_argument(
        "--device",
        type=str,
        default=None,
        help=(
            "Torch device for the RL policy / PPO updates (e.g. 'cuda', 'cpu'). "
    ...
    if policy_device.startswith("cuda") and not env_device_str.startswith("cuda"):
        print(
            f"[b2_z1_wbc_train] WARNING: policy on {policy_device} but Genesis runs "
            f"on {env_device_str}. Prefer `--backend gpu --device cuda` together."
        )

    print(
        f"[b2_z1_wbc_train] Genesis backend: {args.backend} (device={env_device_str})  "
